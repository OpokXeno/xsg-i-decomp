...
public class STool
    ...
    float Objchk_scalez;
    float Objchk_scale;
    float Objchk_scale_cns;
    int Objchk_scale_int;
    Vector4f ObjchkScale;
    boolean Objchk_exec;
    int Objchk_key1;
    int mtnframe;
    boolean keywait_flag;
    Thread CSFade_thread;
    int CSFade_FadeTime;
    int CSFade_base;
    int CSFade_target;
    int[] CSFadeColorScreenParam;
    int msgsignal;
    int cutwait;
    boolean msgflag;
    int MSGtime_min;
    ...
    public STool() {
        ...
        this.Objchk_scale_cns = 0.0f;
        this.Objchk_scale_int = 0;
        this.Objchk_key1 = 256;
        this.mtnframe = 0;
        this.CSFade_thread = Thread.create();
        int[] nArray3 = new int[8];
        nArray3[1] = 1;
        nArray3[2] = 0x100000;
        nArray3[3] = 1614815232;
        this.CSFadeColorScreenParam = nArray3;
        this.msgsignal = 128;
        this.cutwait = 0;
        this.msgflag = true;
    ...
    void CSFade(int n, int n2, int n3) {
        this.CSFade_FadeTime = n3;
        this.CSFade_base = n;
        this.CSFade_target = n2;
        this.CSFade_thread.setTarget(this, "CSFade_srv");
        this.CSFade_thread.start();
    }

    void CSFade_srv() {
        System.println("CSFade start.");
        int n = (this.CSFade_base & 0xFF000000) >> 24;
        int n2 = (this.CSFade_base & 0xFF0000) >> 16;
        int n3 = (this.CSFade_base & 0xFF00) >> 8;
        int n4 = this.CSFade_base & 0xFF;
        Runtime.setRegister(0, n);
        Runtime.setRegister(1, n4);
        Runtime.setRegister(2, n3);
        Runtime.setRegister(3, n2);
        System.println("base(/[$0],/[$1],/[$2],/[$3])");
        int n5 = (this.CSFade_target & 0xFF000000) >> 24;
        int n6 = (this.CSFade_target & 0xFF0000) >> 16;
        int n7 = (this.CSFade_target & 0xFF00) >> 8;
        int n8 = this.CSFade_target & 0xFF;
        int n9 = 0;
        Runtime.setRegister(0, n5);
        Runtime.setRegister(1, n8);
        Runtime.setRegister(2, n7);
        Runtime.setRegister(3, n6);
        System.println("target(/[$0],/[$1],/[$2],/[$3])");
        float f = (float) (n5 - n) / (float) this.CSFade_FadeTime;
        float f2 = (float) (n6 - n2) / (float) this.CSFade_FadeTime;
        float f3 = (float) (n7 - n3) / (float) this.CSFade_FadeTime;
        float f4 = (float) (n8 - n4) / (float) this.CSFade_FadeTime;
        Runtime.setRegister(0, f);
        Runtime.setRegister(1, f4);
        Runtime.setRegister(2, f3);
        Runtime.setRegister(3, f2);
        System.println("d(/[#0],/[#1],/[#2],/[#3])");
        float f5 = n;
        float f6 = n2;
        float f7 = n3;
        float f8 = n4;
        while (n9 != this.CSFade_FadeTime) {
            f5 += f;
            f6 += f2;
            f7 += f3;
            f8 += f4;
            if (f < 0.0f && (float) n5 > f5) {
                f5 = n5;
            }
            if (f > 0.0f && (float) n5 < f5) {
                f5 = n5;
            }
            if (f2 < 0.0f && (float) n6 > f6) {
                f6 = n6;
            }
            if (f2 > 0.0f && (float) n6 < f6) {
                f6 = n6;
            }
            if (f3 < 0.0f && (float) n7 > f7) {
                f7 = n7;
            }
            if (f3 > 0.0f && (float) n7 < f7) {
                f7 = n7;
            }
            if (f4 < 0.0f && (float) n8 > f8) {
                f8 = n8;
            }
            if (f4 > 0.0f && (float) n8 < f8) {
                f8 = n8;
            }
            this.CSFadeColorScreenParam[2] = 0x100000;
            this.CSFadeColorScreenParam[3] = (int) f5 * 0x1000000 + (int) f6 * 65536 + (int) f7 * 256 + (int) f8;
            Runtime.setDefocus(15, 1, this.CSFadeColorScreenParam);
            ++n9;
            System.sleep(1);
        }
        this.CSFadeColorScreenParam[2] = 0x100000;
        this.CSFadeColorScreenParam[3] = n5 * 0x1000000 + n6 * 65536 + n7 * 256 + n8;
        Runtime.setDefocus(15, 1, this.CSFadeColorScreenParam);
        System.println("CSFade End.");
    }
    ...
    void FACE(int n, Chr chr, int n2, float f) {
        chr.mtn(n2, 0, 120, 5, 8, f, false);
        chr.start(4, null);
        System.sleep(n);
        chr.mtn(n2 + 1, 0, 120, 5, 9, f, false);
        chr.start(4, null);
    }
    ...
    void FACE(Chr chr, int n, float f) {
        chr.mtn(n, 8, f, false);
        chr.start(4, null);
    }
    ...
    void MSG(int n, Chr chr, int n2, float f, String string) {
        this.MSG(n, chr, n2, n - 10, f, string);
    }

    void MSG(int n, Chr chr, int n2, int n3, float f, String string) {
        if (this.msg_clear_exec) {
            this.msg_clear.stop();
        }
        this.msg.clear();
    ...
    void Objchk_Keychk_srv() {
        ...
        if ((this.btn1 & 0x10) != 0) {
            ...
            if ((this.btn1 & 0x400) != 0) {
                this.Objchk_dsx /= 10.0f;
                this.Objchk_dsy /= 10.0f;
                this.Objchk_dsz /= 10.0f;
            }
    ...
    void Unit_print(Chr chr) {
        ...
        this.setRegXYZ(this.Objchk_scale * 1000.0f, this.Objchk_scale * 1000.0f, this.Objchk_scale * 1000.0f);
        System.println("setScale1000(/[#0]f,/[#1]f,/[#2]f);");
    }
    ...
    void _xMSG(int n, String string, String string2) {
        this.MSGprintout(string2);
    }
    ...
    void iMSG(int n, Chr chr, int n2, float f, String string) {
        this.iMSG(n, chr, n2, n - 10, f, string);
    }

    void iMSG(int n, Chr chr, int n2, int n3, float f, String string) {
        if (this.msg_clear_exec) {
            this.msg_clear.stop();
        }
        this.msg.clear();
        ...
        System.sleep(n3);
        chr.mtn(n2 + 1, 0, 120, 5, 8, f, false);
        chr.start(4, null);
        if (n > n3) {
            System.sleep(n - n3);
        }
    ...
    void mtnStop(Chr chr) {
        chr.mtn(Integer.MIN_VALUE, 0, 1.0f, false);
    }
    ...
    void sFACE(int n, Chr chr, int n2, float f) {
        chr.mtn(n2, 0, 120, 5, 9, f, false);
        chr.start(4, null);
        System.sleep(n);
        chr.mtn(n2 + 1, 0, 120, 5, 9, f, false);
        chr.start(4, null);
    ...
    void sFACE(Chr chr, int n, float f) {
        chr.mtn(n, 9, f, false);
        chr.start(4, null);
    }
    ...
    void sMSG(int n, Chr chr, int n2, float f, String string) {
        this.sMSG(n, chr, n2, n - 10, f, string);
    }

    void sMSG(int n, Chr chr, int n2, int n3, float f, String string) {
        if (this.msg_clear_exec) {
            this.msg_clear.stop();
        }
        this.msg.clear();
        chr.mtn(n2, 0, 120, 10, 9, f, false);
        chr.start(4, null);
        if (this.msgflag) {
            this.MSGprintout(string);
            this.msg.print(string);
    ...
    void xMSG(int n, String string, String string2) {
        this.MSGprintout(string2);
    }
