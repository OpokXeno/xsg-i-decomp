...
class ST0324
    ...
    String[] SHION = new String[]{"/[label(Shion)]", "KOS-MOS broke the window, so we can't access the Realian Infirmary.", "/[waitkey(64)]/[close()]"};
    String[] KOSMOS = new String[]{"/[label(KOS-MOS)]", "The area ahead was destroyed during the earlier battle with Gnosis.\n", "/[waitkey(1)]/[clear()]", "I think it would be best if you board an escape pod.", "/[waitkey(64)]/[close()]"};
    String[] SHION_1 = new String[]{"/[label(Shion)]", "Now that I think about it, that switch should open the partition.", "/[waitkey(64)]/[close()]"};
    String[] SHION_2 = new String[]{"/[label(Shion)]", "We wouldn't have had to take the long way around if only the partition was open. ", "/[waitkey(64)]/[close()]"};
    String[] KOSMOS_2 = new String[]{"/[label(KOS-MOS)]", "This sealed partition prevented me from reaching you sooner.", "/[waitkey(64)]/[close()]"};
    ...
    void EV_Camera01() {
        this.camEV = Camera.create(1);
        this.camEV.setTranslate(16.879f, 3.298f, 14.61f);
        this.camEV.setRotate(-26.655f, 38.27f, 0.0f);
    ...
    public void KickEvent(int n, int n2) {
        ...
        if (n2 == 2) {
            switch (n) {
                case 100: {
                    if (Runtime.getFlags(3019, 1) != 0) return;
                    Runtime.setPlayerControl(false);
                    ...
                    this.cam0.setMode(-1);
                    this.EV_Camera00_01a();
                    Runtime.setFlags(3019, 1, 1);
                    this.EV_Camera00_01();
                    System.sleep(90);
                    this.EV_Camera00_01b();
                    System.sleep(30);
                    this.cam0.setMode(0);
                    Runtime.setPlayerControl(true);
        ...
        if (n2 == 3) {
            switch (n) {
                case 100: {
                    ...
                    this.buttonB_flg = 1;
                    if (Runtime.getFlags(3018, 1) != 1) return;
                    Runtime.setPlayerControl(false);
                    this.win = Window.create();
                    this.win.setSize(4, 45);
                    this.win.setLocation(15, 305);
                    if (Runtime.getLeader() == 1) {
                        this.win.print(this.SHION, 0);
                    } else {
                        this.win.print(this.KOSMOS, 0);
                    }
                    System.waitFor(this.win);
                    Runtime.setPlayerControl(true);
                    this.buttonB_flg = 0;
        ...
        switch (n) {
            case 100: {
                if (Runtime.getFlags(3063, 1) != 0) return;
                Runtime.setPlayerControl(false);
                Runtime.enable(65536);
                ...
                if (Runtime.getLeader() == 1) {
                    ...
                    this.win.print(this.KOSMOS_2, 0);
                }
                System.waitFor(this.win);
                Runtime.setFlags(3063, 1, 1);
                Runtime.setPlayerControl(true);
                Runtime.disable(65536);
            }
        }
        ...
        switch (n) {
            ...
            case 3: {
                if (Runtime.getFlags(50, 1) == 0) {
                    Runtime.setFlags(50, 1, 1);
                    Runtime.jumpEvent(1390);
                    break;
                }
            ...
            case 4: {
                Runtime.setFlags(3049, 1, 1);
                Runtime.jumpCF(65716, 1);
                break;
            }
        }
    }
    ...
    void init() {
        ...
        this.player.setTranslate(-7.5f, 2.0f, 19.0f);
