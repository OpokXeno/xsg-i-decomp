...
class ST0530
    ...
    public void entered(int n) {
        ...
        switch (n) {
            case 0: {
                if (this.S2030 == 1) {
                    Runtime.jumpCF(520, 8);
                    break;
                }
                if (this.S2028 == 1) {
                    Runtime.jumpCF(670, 8);
                    break;
                }
                Runtime.jumpCF(520, 8);
                break;
    ...
    class Mapunits
        ...
        void automatic_door() {
            ...
            int n3 = 45;
            while (true) {
                ST0530.this.player.getTranslate();
                f2 = (0.0f - ST0530.this.player.px) * (0.0f - ST0530.this.player.px) + (-7.875f - ST0530.this.player.pz) * (-7.875f - ST0530.this.player.pz);
                if (f2 < 4.0f && ST0530.this.doormove == 2) {
                    ST0530.this.doormove = 1;
                }
                if (f2 > 9.0f) {
                    ST0530.this.doormove = 2;
                }
                if (ST0530.this.doormove == 1 && f < (float) (n2 + n)) {
                    if (f == 0.0f) {
                        Sound.effectPlay(196713);
                    }
                    if (f <= (float) n2) {
                        ST0530.this.doorR1.setRotate(0.0f, f * (float) n3 / (float) n2, 0.0f);
                        ST0530.this.doorL1.setRotate(0.0f, -f * (float) n3 / (float) n2, 0.0f);
                    }
                    if (f >= (float) n) {
                        ST0530.this.doorR2.setRotate(0.0f, (f - (float) n) * (float) n3 / (float) n2, 0.0f);
                        ST0530.this.doorL2.setRotate(0.0f, (-f + (float) n) * (float) n3 / (float) n2, 0.0f);
                    }
                    f += 1.0f;
                }
                if (ST0530.this.doormove == 2 && f > 0.0f) {
                    if ((f -= 1.0f) == (float) n2) {
                ...
                if (f > (float) (n2 + n)) {
                    ST0530.this.doormove = 0;
                }
                if (f <= 0.0f) {
                    ST0530.this.doormove = 0;
                }
                if (f > (float) n2) {
                    ST0530.this.tA.setTranslate(0.0f, -3.5f, -7.875f);
                } else {
                    ST0530.this.tA.setTranslate(0.0f, -1.5f, -7.875f);
                }
                System.sleep(1);
            }
        }
