...
/*
 * Defined by ov12/tu045 (src/ov12/rg_draw.c, unpublished body); this TU only
 * forwards the pointer to RgDrawViewInit, so an incomplete type is enough
 * here (same convention as src/ov12/rg_draw.h and src/ov12/rg_player.h).
 */
...
typedef struct SelType {
    ...
    unsigned char unmodeled_628[12];
    int confrict[6];
} SelType;
