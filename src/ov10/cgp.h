/*
 * TU-local declarations of ov10/tu008 (src/ov10/cgp.c).
 */

#ifndef SRC_OV10_CGP_H
#define SRC_OV10_CGP_H
...
/*
 * CardSaveData.flags bit CGPRotStageSub tests (0x00a2a698 `lw $3,0xC($4)`,
 * `lbu $2,0($3)`, `andi $2,$2,0x10`). CardMainProc (ov10/tu001) also toggles
 * bit 0x20 from the title menu to select the large card models
 * (0x00a03ff0..0x00a04008, tested at 0x00a04014) and sets bit 0x40 once the
 * promotional card was awarded (0x00a0610c `andi $2,$2,0x40`, 0x00a06124 `ori`).
 */
#define CARD_SAVE_ROTATE_STAGE 0x10
...
/* CardGameWork.flags bit CGPSetErrorMessPlus sets. */
#define CGP_FLAG_ERROR_PLUS 0x0002u

extern char *OLMessTbl[];
extern char *CardPlayTMessList[];
extern char *PermMessTbl[];

/*
 * Per-card definition CGPCommFinishSub indexes by card id. Only the cost
 * field it reads (`lbu` at +0x10) is evidenced; the element stride (0x24
 * bytes, from the index*9*4 address arithmetic) fixes the struct size.
 */
typedef struct CardDef {
    u8 unmodeled_00[0x10];
    u8 cost;                        /* +0x10: CardPlayCostCard is called this many times */
    u8 unmodeled_11[0x24 - 0x11];
} CardDef;

/*
 * Source CGPCommFinishSub reads its CardDef table from. Only the table
 * pointer it dereferences (`lw` at +0x08) is evidenced.
 */
typedef struct CGPCardDefSource {
    u8 unmodeled_00[8];
    CardDef *defs;                  /* +0x08 */
} CGPCardDefSource;

/*
 * Per-player play record CGPCommFinishSub receives. Its first 8 bytes and
 * cards[40] line up with CardHand's cards, but this function also touches
 * fields beyond CardHand's currently evidenced span, so it is modeled as
 * its own partial type until CardHand's growth is reconciled.
 */
typedef struct CardPlayHand {
    u8 unmodeled_00[8];
    s16 cards[40];                  /* +0x08 */
    u8 unmodeled_58[0xF8 - 0x58];
    s16 junkPile[40];                /* +0xF8: indexed by CardPlayJunkCnt's result */
    u8 unmodeled_148[0x198 - 0x148];
    s16 lastCardValue;               /* +0x198: card id CGPCommFinishSub is finishing */
} CardPlayHand;

/*
 * CardPlayCleanHand/CardPlayCostCard/CardPlayJunkCnt (ov10/tu002, still
 * assembler) all take the per-player play record CGPCommFinishSub touches;
 * declared locally since their defining TU has not published a header yet.
 */
extern void CardPlayCleanHand(CardPlayHand *work);
extern void CardPlayCostCard(CardPlayHand *work);
extern int CardPlayJunkCnt(CardPlayHand *work);
...
/*
 * Attack type of a card in play (ov10/tu002, 0x00a0cec0, still assembler);
 * type 4 spends the card's boosts instead of doubling its power.
 */
extern s32 CardChkAttackType(CardDefinition *definitions, CardPowerCard *card);
