...
def main():
    ...
    for t in manifest["tus"]:
        ...
        head = [f"/*\n * {a.unit.upper()} original TU {t['ordinal']}: {t['text'][0]}..{t['text'][1]}"
                ...
                f" * Name: {t['name']}\n"
                f" * Accepted C: {', '.join(c_funcs) or '-'}\n"
                f" * Accepted standalone EE assembly: {', '.join(asm_accepted) or '-'}\n"
                " * Generated by tools/tu/gen_ovl_src.py from the accepted\n"
                " * unit sources (function text verbatim) and splat INCLUDE_ASM bodies.\n"
                + "".join(f" * Header divergence ({d['name']}): {d['summary']}\n"
                          for d in adapt.get(tu, {}).get("header_divergence", []))
                + " */",
                '#include "common.h"']
